<template>
  ...
  >
    ...
    <form
      ...
      class="max-w-md w-full bg-accent/40 text-accent-foreground p-8 rounded-md border shadow-md shadow-card"
    >
      <FieldSet>
        <FieldGroup>
          <FieldTitle class="flex flex-col gap-2 items-start">
            <h1 class="text-3xl font-semibold">Welcome <span>Admin</span></h1>

            <FieldDescription>
              Enter your email and password to sign in to admin account
            </FieldDescription>
          </FieldTitle>
...
const handleSubmit = async () => {
  if (!form.email.split("@")[1]?.startsWith("admin")) {
    toast.error("You cannot access user account from this page", {
      class:
